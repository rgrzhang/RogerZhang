...
plot_pvals <- function( tmax=6, # end time for numerical integration of the ODE
  ...
  legend("topleft",legend=vary_p,col=1:length(vary_p),lty=1:length(vary_p))
}

if (!interactive()) pdf("myplot.pdf")
plot_pvals()
plot_pvals(ylim=c(0.02,0.03))
