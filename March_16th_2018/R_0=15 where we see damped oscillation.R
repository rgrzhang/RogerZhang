...
SIR.vector.field <- function(t, vars, parms) {
  ##SIR.vector.field <- function(t, vars=c(S=S,I=I,R=R,Q=Q,X=X), parms=c(R_0=R_0,gamma=gamma,delta=delta,mu=mu,p=p)) {
    ...
    dV <- gamma*R_0*S*I - gamma*I-mu*I #cumulative infected cases with normal vaccination
    vec.fld <- c(dS=dS, dI=dI, dR=dR, dQ=dQ, dX=dX, dV=dV)
    return(list(vec.fld)) # ode() requires a list
  })
}

draw.soln <- function(ic=c(S=1,I=0,R=0,Q=0,X=0,V=0), tmax=1,
...
for (i in 1:length(vary_p)) {
  draw.soln(ic=c(S=S0,I=I0,R=R0,Q=Q0,X=X0,V=V0), tmax=tmax,
            func=SIR.vector.field,
            parms=c(R_0=20,gamma=1,delta=0,mu=1/80,p=vary_p[i]),
            col=i,
