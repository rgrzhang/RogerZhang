...
\caption{Figure a-d illustrate $(\R_0,\epsilon)$ planes with different $p$ values. \\ a) $p=0$, b) $p=0.2$,c) $p=0.5$, d) $p=0.8$}
\end{minipage}
...
\begin{figure}[h]
  ...
  \caption{Determination of time taken to reach equilibrium.  The horizontal line at $I=10^{-6}$ is the threshold we define to be ``at equilibrium''.}
\label{figure:0.0001}
\end{figure}
...
\david{You must have title for the legend so we know what is the curves refer to.}}
...
We found that, if $p$ is small, though it gains advantage faster, the number of deaths actually stays very close to non-intentional infection. In another words, although the advantage is obtained quickly, its magnitude is too small to be significant.  With this in mind, we will consider one intentional infection strategy ($p$ value) to be ``More advantageous'' than another if mortality with the strategic value of $p$ is at least 10\% lower than mortality with no intentional infection ($p=0$)
...
\begin{figure}[H]
  ...
  \caption{($\gamma,p$) plane when $\R_{0,V}=2.5, \R_{0,I}=4.5$. The shaded area is the region where $\Re(\lambda_1)<0$}
\end{figure}
...
Previous study have shown that, for a similar model which does not consider death, a significant decrease in final size is expected \cite{nuismer2018controlling}, for a transmissible vaccine, even when $\R_{0,V}$ is less than 1, e.g. $\R_{0,V}=0.5$, we would expect similar results for our model, the difference is, with the addition of birth and death rate, since our system have shown that the disease cannot be completely eradicated, there will always be newly infected cases, therefore, we do not have a final size estimation. 
