...
SIR.vector.field <- function(t, vars, parms) {
  ##SIR.vector.field <- function(t, vars=c(S=S,I=I,R=R,Q=Q,X=X), parms=c(R_0=R_0,gamma=gamma,delta=delta,mu=mu,p=p)) {
    ...
    dS <- mu -(gamma+mu)*R_0*S*(I_N+I_T) -p*S -mu*S# dS/dt
    dQ <- (gamma+mu)*R_0*S*(I_N+I_T)+p*S + mu*p # cumulative cases
    dX <- (gamma+mu)*R_0*S*I_N #cumulative normally infected cases
    dI_T <- p*S+(gamma+mu)*R_0*S*I_T - gamma*I_T-mu*I_T # intensional infected cases, and cases which is infected by them
    dI_N <- (gamma+mu)*R_0*S*I_N - gamma*I_N-mu*I_N # Normally infected cases
...
plot(0,0,xlim=c(0,tmax),ylim=c(0,ymax),
     type="n",main="Normally infected cases",xlab="Time (t)",ylab="I_N",las=1)
