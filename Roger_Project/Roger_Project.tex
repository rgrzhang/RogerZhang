...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{Figures/Model_1_2_distance_500.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{Figures/TTE.pdf}
...
It is necessary to investigate the dynamics when EE is unstable. Recall the other equilibrium which is only valid for some parameter values. We discovered that, at the boundary condition in \autoref{Figure_gamma_p}, our first equilibrium loses stability. But we also found that the second equilibrium becomes biologically meaningful at the same time. In order to identify whether the second equilibrium is stable, we compute the eigenvalues of the Jacobian again,

$\Re(\lambda_2)$ and $\Re(\lambda_3)$ are still both negative.

\begin{equation}
\Re(\lambda_1)=-1+\frac{\R_{0,V}}{\R_{0,I}}\,.
\end{equation}

With our intuition that $\R_{0,V}<\R_{0,I}$, we can conclude that $\Re(\lambda_1)<0$. Therefore, when the first equilibrium is unstable, the system approaches another equilibrium. Hence, the stability of the system switches between two equilibria at the boundary of \autoref{Figure_gamma_p}.
...
Figures used in this document were created by ``R'' or ``Mathematica (Version 11.3)''. The files used to construct figures are under the folder named ``Supplementary material''
