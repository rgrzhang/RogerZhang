...
draw.soln <- function(ic=c(S=S,V=V,I=I), tmax=20,
...
for (i in 1:runs){
  ...
  mydiff <- draw.soln(func=SIR.vector.field,
                      parms=c(R_0=4.5,gamma=1/(22/365),p=1,mu=1/50))
  Df <- mydiff
  Shat <- 1/4.5-2*1/(3.5+(3.5^2+4*4.5*1)^(0.5))
  Vhat <- (11/9136*(3.5+(3.5^2+4*4.5*1)^(0.5)))/(9)
  Df$dS <- Df$S-Shat
  Df$dV <- Df$V-Vhat
